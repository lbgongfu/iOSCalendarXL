...
- (NSArray<NSString*> *)getEraAndYearOfLunar: (int)year {
    std::pair<int, int> p = ChineseCalendarDB::GetEraAndYearOfLunar(year);
    return [NSArray<NSString*> arrayWithObjects:[[NSString alloc] initWithFormat:@"%d", p.first], [[NSString alloc] initWithFormat:@"%d", p.second], nil];
}
