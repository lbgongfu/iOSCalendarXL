...
//    NSString *minute = self.minute.length == 3 ? [NSString stringWithFormat:@"%ld", self.minute.integerValue] : [NSString stringWithFormat:@"0%ld", self.minute.integerValue];
    ...
    if ([self.delegate respondsToSelector:@selector(LunarDatePickerViewSaveBtnClickDelegate:)]) {
        [self.delegate LunarDatePickerViewSaveBtnClickDelegate:self.selectStr];
    }
}
/// 取消按钮点击方法
- (void)cancelBtnClick {
